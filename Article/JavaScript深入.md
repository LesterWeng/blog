...
> 本文部分参考自 [JavaScript 深入系列](https://github.com/mqyqingfeng/Blog)
...
> 已过期，新的执行上下文包括：`变量环境`、`词法环境`(let、const)、`outer`、`this`
...
在执行代码前，`v8` 会在代码执行前进行一次`预扫描`，分析出`内部函数`需要使用到的`外部变量`，当执行`内部函数`时，所有`外部函数`(或外部函数的外部函数)内被`内部函数`(或内部函数的内部函数...)使用了的变量都就会以`执行上下文`为单位形成一个个`闭包对象`，存储在`堆内存`中，即使外部函数执行完销毁了执行上下文，闭包对象依然会存在堆内存中（可查看 chrome devtool debugger `scope`）
