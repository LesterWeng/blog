...
在执行代码前，`v8` 会在代码执行前进行一次`预扫描`，分析出`内部函数`需要使用到的`外部变量`，当执行内部函数时，如果需要使用到外部变量，外部变量就会以`执行上下文`为单位形成一个个`闭包对象`，存于`堆`中，即使外部函数执行上下文已被销毁，闭包对象依然会存在堆内存内（可查看 chrome devtool debugger `scope`）
